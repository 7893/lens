...
describe('Search API Route', () => {
  ...
  it('streams search results via SSE when stream=true', async () => {
    mockAi.run.mockResolvedValue({ data: [[0.1, 0.2, 0.3]] });
    mockVectorize.query.mockResolvedValue({
      matches: [{ id: 'photo123', score: 0.85 }],
    });
    ...
    const executionCtx = { waitUntil: vi.fn(), passThroughOnException: vi.fn() };
    const res = await search.request('/?q=cat&stream=true', {}, env, executionCtx as unknown as ExecutionContext);

    expect(res.status).toBe(200);
    expect(res.headers.get('content-type')).toContain('text/event-stream');
    const text = await res.text();
    expect(text).toContain('stage');
    expect(text).toContain('photo123');
  });
});
