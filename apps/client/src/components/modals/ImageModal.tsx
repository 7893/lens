...
export function ImageModal({ image, score, onClose }: { image: ImageResult; score?: number; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-4 animate-fade-in" onClick={onClose}>
      <div className="relative max-w-6xl w-full max-h-[92vh] flex flex-col md:flex-row bg-white rounded-3xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 z-10 p-2 bg-black/40 hover:bg-black/60 rounded-full text-white transition-all backdrop-blur-md">
          <X className="w-5 h-5" />
        </button>

        <div className="md:w-[65%] bg-neutral-900 flex items-center justify-center min-h-[300px]" style={{ backgroundColor: detail?.color || '#1a1a1a' }}>
          <img src={image.url} alt={image.caption || ''} className="max-w-full max-h-[85vh] object-contain shadow-inner" />
        </div>
        ...
        <div className="md:w-[35%] p-7 overflow-y-auto space-y-6 max-h-[92vh] bg-white">
          ...
          <div className="space-y-4">
            {detail?.sponsorship && (
              <div className="flex items-center gap-3 p-3.5 bg-amber-50 rounded-2xl border border-amber-100">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <a href={detail.sponsorship.url || '#'} target="_blank" rel="noopener" className="text-sm font-semibold text-amber-900 hover:underline truncate block">
            ...
            {detail?.photographer && (
              <div className="flex items-center gap-3.5">
                <img src={detail.photographer.profileImage || ''} alt="" className="w-12 h-12 rounded-2xl object-cover ring-2 ring-gray-50" />
                <div className="flex-1 min-w-0">
                  <a href={detail.photographer.profile || '#'} target="_blank" rel="noopener" className="font-bold text-gray-900 hover:text-blue-600 transition-colors">
          ...
          {/* Description & AI */}
          <div className="space-y-4">
            {detail?.description && <p className="text-sm text-gray-700 leading-relaxed font-medium">{detail.description}</p>}
            ...
            <div className="bg-blue-50/50 p-4 rounded-2xl space-y-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {(detail?.ai?.tags || []).map(t => (
                  <span key={t} className="text-[10px] px-2.5 py-1 bg-white text-blue-500 rounded-lg shadow-sm font-medium">#{t}</span>
                ))}
              </div>
            </div>
          </div>
            ...
            {detail?.exif && (
              <div className="space-y-2 col-span-2 bg-gray-50 p-4 rounded-2xl">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                  ...
                  {detail.exif.exposure && <span className="flex items-center gap-1"><Clock className="w-3 h-3"/>{detail.exif.exposure}</span>}
          ...
          <div className="text-[10px] text-gray-400 space-y-1 pt-4 border-t border-dashed">
            <div className="flex justify-between"><span>Resolution</span><span className="text-gray-600 font-medium">{detail?.width || image.width} × {detail?.height || image.height}</span></div>
            {score !== undefined && <div className="flex justify-between"><span>Semantic Score</span><span className="text-blue-500 font-bold">{(score * 100).toFixed(1)}%</span></div>}
            <div className="flex justify-between"><span>Engine Model</span><span className="text-gray-600">{detail?.ai?.model || 'llama-4-scout'}</span></div>
          </div>
          
          {detail?.source && (
            <a href={detail.source} target="_blank" rel="noopener" className="flex items-center justify-center gap-2 py-3 w-full bg-gray-900 text-white rounded-2xl text-xs font-bold hover:bg-black transition-colors">
