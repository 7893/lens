...
## 1. 宏观架构：单 Worker 双管道与事件驱动逻辑

Lens 的系统设计遵循 **“生产与消费逻辑隔离”** 的原则。系统被逻辑划分为两条独立的动态流水线，它们共用底层的存储矩阵（D1, R2, Vectorize），运行在同一个 `lens` Worker 内，通过不同的触发入口（HTTP fetch / Cron scheduled / Queue consumer）完全解耦。
...
- **线性对撞抓取 (Linear Ingestion)**：基于每小时一次的定时脉冲，系统会动态探测 Unsplash 的最新动态。
