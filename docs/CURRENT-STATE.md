...
本地工具链于 2026-09-25 对齐 jpa：Node.js 26.10.0、pnpm 12.6.0、Terraform 1.16.3。
版本约束及 CI 配置已同步；此项尚未推送或部署，不改变下述线上状态。

本地安全修复：管理接口统一验证 `INTERNAL_API_SECRET`，无开发模式绕过；新增管理限流及补偿批量上限。尚未部署，部署前必须配置新 secret。

2026-09-26 workflow 恢复补充（取代下文手动部署描述）：按用户要求恢复原有 `main` push
检查通过后自动部署；Pull Request 只检查。正式仓库为 `7893/lens`。
本补充记录触发规则恢复，实际部署结果以 GitHub Actions 为准。

2026-09-26 开源交付整改：公开源码使用 `7893/lens-clean`；根目录环境模板补齐
`INTERNAL_API_SECRET`。公共 push/PR 只执行检查，生产部署改为手动运行 CI 并显式选择
`deploy`。这些调整不改变线上服务；本次不进行生产部署。
