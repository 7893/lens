...
更新日期：2026-09-19
状态：现行
适用范围：D1、R2、Vectorize、KV 异构存储契约与多维索引治理
...
- **智力版本标记 (`ai_model`)**：
  这是实现 **“存量数据自我进化”** 的核心。系统会周期性审计数据库，所有 `ai_model` 不等于最新旗舰版（如 `llama-4-scout`）的记录，都会被自动推入进化队列进行重刷。
...
为了弥补向量搜索在处理“精确词”（如品牌名、相机型号、地标）时的短板，Lens 引入了 SQLite FTS5 全文搜索虚拟表 `images_fts`。

### 2.1 混合搜索架构

- **虚拟表同步**：通过 D1 Triggers（触发器），`images` 表的增删改会自动同步到 `images_fts`。
- **搜素收益**：系统能够实现毫秒级的关键词匹配，并与向量搜索结果进行融合排名。
